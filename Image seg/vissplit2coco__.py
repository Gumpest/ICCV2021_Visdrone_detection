...
def convert_to_cocodetection(dir, output_dir):
    train_test_dir = os.path.join(dir, "VisDrone2019-DET-train-test-split")
    # val_dir = os.path.join(dir, "VisDrone2019-DET-valsplit")
    # test_dir = os.path.join(dir, "VisDrone2019-DET-test-devsplit")
    train_test_annotations = os.path.join(train_test_dir, "annotations")
    # val_annotations = os.path.join(val_dir, "annotations")
    # test_annotations = os.path.join(test_dir, "annotations")
    train_test_images = os.path.join(train_test_dir, "images")
    # val_images = os.path.join(val_dir, "images")
    # test_images = os.path.join(test_dir, "images")
    id_num = 0

    categories_dict = {"pedestrian": '0', "people": '1', "bicycle": '2',
              "car": '3', "van": '4', "truck": '5',
              "tricycle": '6', "awning-tricycle": '7',
              "bus": '8', "motor": '9'}
    ...
    for mode in ["train-test"]:
        ...
        if mode == "train-test":
            set = os.listdir(train_test_annotations)
            annotations_path = train_test_annotations
            images_path = train_test_images
        elif mode == "test":
        ...
        for i in tqdm(set):
            ...
            for line in f.readlines():
                ...
                bbox_xywh = [int(line_list[0].replace('.0','')), int(line_list[1].replace('.0','')), int(line_list[2].replace('.0',''))-int(line_list[0].replace('.0','')), int(line_list[5].replace('.0',''))-int(line_list[1].replace('.0',''))]
                annotation["image_id"] = name
                # annotation["score"] = line_list[4]
                annotation["bbox"] = bbox_xywh
                annotation["category_id"] = int(categories_dict[line_list[8]])+1
...
if __name__ == '__main__':
    convert_to_cocodetection(r"/home/zy/YOLOv5/VisDrone/",r"data/VisDronesplit__")
